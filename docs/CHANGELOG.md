...
- [改进] 筹码分布链路新增最近 3 个交易日成功快照兜底，东财免费接口瞬时失败时优先复用最近可用筹码而不是直接整块缺失。
...
- [改进] 问股页优化手机端高度、消息气泡、操作按钮和输入区布局；移动浏览器对长连接流式响应不稳定时自动使用兼容的缓冲请求，避免手机端误报请求失败。
- [修复] 修复手机端分析与问股页面使用动态视口高度，并移除内容底部多余滚动间距，避免页面到底后仍能滑出空白区域。
- [修复] 问股页手机端锁定背景页面的滚动与回弹，仅保留消息列表自身滚动，避免整页背景跟随手势移动。
- [修复] 针对部分手机 WebView 仍可拖出空白的问题，将问股页背景固定在视口内，同时保留消息列表的内部滚动。
- [修复] 禁止问股页背景接收页面级下滑手势，避免部分手机 WebView 在内容底部继续拖出空白。
- [修复] 统一修复其他手机页面的外层回弹和底部空隙；回测与 404 页面在内容一屏可见时不再产生额外页面滚动。
